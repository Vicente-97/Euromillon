...
public class Historial {
	...
	public Integer comprobarAciertos(LocalDate fecha, Combinacion combinacion){
		
		Iterator<Integer> numRecibido = combinacion.getNumeros().iterator();
		Iterator<Integer> estrellasRecibido = combinacion.getEstrellas().iterator();
		List<Integer> numeros = new ArrayList();
		List<Integer> estrellas = new ArrayList();
		Integer contador= 0;
		if(this.historial.containsKey(fecha)) {
			
			while(numRecibido.hasNext() && estrellasRecibido.hasNext()) {
				numeros.add(numRecibido.next());
